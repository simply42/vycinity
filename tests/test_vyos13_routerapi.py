...
class Vyos13RouterAPITest(TestCase):
    ...
    def test_list_routers_good(self):
        ...
        self.assertEqual(str(self.test_router.id), content['results'][0]['id'])
        self.assertEqual('testrouter', content['results'][0]['name'])
        self.assertEqual('1.2.3.4', content['results'][0]['loopback'])
        self.assertFalse(content['results'][0]['deploy'])
        self.assertListEqual(['interfaces', 'ethernet', 'eth1'], content['results'][0]['managed_interface_context'])
        self.assertEqual('dev123', content['results'][0]['token'])
        self.assertEqual('1234567890abcdef0', content['results'][0]['fingerprint'])
